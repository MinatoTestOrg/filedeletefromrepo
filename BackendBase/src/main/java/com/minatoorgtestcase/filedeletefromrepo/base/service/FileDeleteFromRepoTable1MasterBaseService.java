...
public class FileDeleteFromRepoTable1MasterBaseService<T extends FileDeleteFromRepoTable1MasterBase> extends BaseBusinessLogic<T>
		implements IFileDeleteFromRepoTable1MasterBaseService<T> {
		
	private static final Logger LOGGER = LoggerFactory.getLogger(FileDeleteFromRepoTable1MasterBaseService.class.getName());
	...
	@Autowired
	private  FileDeleteFromRepoTable1MasterPerimeterBaseImpl filedeletefromrepotable1masterPerimeterBaseImpl;
	
	
	public FileDeleteFromRepoTable1MasterBaseService(Class<T> modelClass) {
		super(modelClass);
	...
	public void onAfterSaveDB(T modelObj) {
	  changelogService.createChangeLog("FileDeleteFromRepoTable1Master", modelObj.getSid(), Constants.SAVED, modelObj);
	}
	...
	public void onAfterUpdateDB(T modelObj) {
	  changelogService.createChangeLog("FileDeleteFromRepoTable1Master", modelObj.getSid(), Constants.UPDATED, modelObj);
	}
	...
	public void onAfterDeleteDB(T modelObj) {
	  changelogService.createChangeLog("FileDeleteFromRepoTable1Master", modelObj.getSid(), Constants.DELETED, modelObj);
	}	
	...
	protected IPerimeterManager<T> getPerimeterManager() {
		return filedeletefromrepotable1masterPerimeterBaseImpl;
	}
}